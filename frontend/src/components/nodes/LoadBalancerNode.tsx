...
export default function LoadBalancerNode({ id, data }: { id: string; data: any }) {
    ...
    return (
        <div className={`px-4 py-3 shadow-xl rounded-lg bg-gray-800 border-2 text-gray-100 flex flex-col gap-2 transition-all duration-200 ${
            isExpanded ? 'border-green-500 min-w-[260px]' : 'border-gray-700 min-w-[200px]'
        }`}>
            <Handle type="target" position={Position.Top} className="w-3 h-3 bg-green-400 border-none" />
            ...
            <div className="text-xs text-gray-400 flex justify-between bg-gray-900 px-2 py-1 rounded">
                <span>Routing:</span>
                <span className="text-green-300 font-mono">{data.latency ?? 0}ms</span>
            </div>
                        ...
                        <div className="flex flex-col gap-1">
                            {data.data_steps.map((step: string, i: number) => (
                                <div key={i} className="flex gap-1.5 items-start text-xs">
                                    <span className="text-green-400 font-mono mt-0.5 flex-shrink-0">{i + 1}.</span>
            ...
            <Handle type="source" position={Position.Bottom} className="w-3 h-3 bg-green-400 border-none" />
        </div>
    );
}
